...
def complete_provenance(records: list[dict], required_fields: tuple[str, ...]) -> bool:
    """Return whether every usable source record contains its provenance fields."""
    return bool(records) and all(all(record.get(field) not in (None, "") for field in required_fields) for record in records)
