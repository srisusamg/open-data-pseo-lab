...
The repository now also contains **AI Model Economics**, a second quality-gated site with a 44-model, 15-provider canonical catalog spanning proprietary APIs, open-weight and local models, and product-only models. Its independent, cohort-safe rankings cover intelligence, reasoning, coding, costs, context, open weights, workload value, latency, throughput, and a Pareto price-performance frontier—without an arbitrary overall score. Its generated site is isolated at `site/ai-model-economics/`; OpenData Atlas behavior and URLs remain unchanged.
