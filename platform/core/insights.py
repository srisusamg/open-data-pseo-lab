...
def _comparison_id(comparison: Mapping[str, Any]) -> str | None:
    if not comparison:
        return None
    a, b = comparison.get("entity_a", {}), comparison.get("entity_b", {})
    return f"{a.get('id', '')}:{b.get('id', '')}"
...
def generate_comparison_candidates(context: InsightContext) -> list[dict]:
    ...
    metrics_by_slug = {metric["metric_slug"]: metric for metric in context.metrics}
    leads = []
    for slug in ("population", "gdp", "gdp-per-capita", "internet-users"):
        metric = metrics_by_slug.get(slug)
        if metric and metric["leader"]:
            leads.append({"indicator_name": metric["metric_name"],
                          "leader": metric["leader"]["entity_name"]})
    if leads:
        shown = leads[:2]
        candidates.append(_candidate(
    ...
    for metric in context.metrics:
        ...
        if period["growth_leader"]:
            gap = abs(period["entity_a"]["percentage_change"] - period["entity_b"]["percentage_change"])
            growth.append((gap, metric, period))
    if growth:
        _, metric, period = max(growth, key=lambda item: (item[0], item[1]["metric_slug"]))
        evidence = {"period_years": 10, "leader": period["growth_leader"]["entity_name"]}
        candidates.append(_candidate(
            "comparison", "growth_leader", entity_id=entity_id,
            metric_id=metric["metric_id"], metric_name=metric["metric_name"],
            value=evidence["leader"], evidence=evidence,
            dedupe_key=f"growth:{metric['metric_id']}",
        ))

    gaps = [metric for metric in context.metrics if metric["relative_gap"] is not None]
    if gaps:
        metric = max(gaps, key=lambda item: (item["relative_gap"], item["metric_slug"]))
        candidates.append(_candidate(
            "comparison", "largest_relative_gap", entity_id=entity_id,
            metric_id=metric["metric_id"], metric_name=metric["metric_name"],
            value=metric["relative_gap"], evidence={"relative_gap": metric["relative_gap"]},
            dedupe_key=f"relative-gap:{metric['metric_id']}",
        ))
    ...
    if changing:
        ...
        candidates.append(_candidate(
            "comparison", kind, entity_id=entity_id, metric_id=metric["metric_id"],
            metric_name=metric["metric_name"],
            value=gap["end_relative_gap"] - gap["start_relative_gap"], evidence=gap,
            dedupe_key=f"gap-change:{metric['metric_id']}",
        ))
...
def _format_value(value: float, style: str) -> str:
    from platform.core.rendering import format_value
...
def render_summary(insights: list[dict],
    ...
    if normalized and normalized.context_type == "comparison" and len(sentences) < 2:
        a = normalized.subject["entity_a"]["name"]
        b = normalized.subject["entity_b"]["name"]
